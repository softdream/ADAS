  ...
  def serialize(self, buff):
    ...
    try:
      ...
      for val1 in self.obstacle:
        ...
        buff.write(_struct_I.pack(length))
        pattern = '<%si'%length
        buff.write(struct.Struct(pattern).pack(*val1.property_type))
        length = len(val1.property_conf)
        buff.write(_struct_I.pack(length))
        pattern = '<%sf'%length
        buff.write(struct.Struct(pattern).pack(*val1.property_conf))
        length = len(val1.tracking_fmap)
        buff.write(_struct_I.pack(length))
        pattern = '<%sb'%length
        buff.write(struct.Struct(pattern).pack(*val1.tracking_fmap))
      _x = self.conf_scale
      buff.write(_get_struct_f().pack(_x))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      for i in range(0, length):
        ...
        val1.property_type = s.unpack(str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        pattern = '<%sf'%length
        ...
        val1.property_conf = s.unpack(str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        pattern = '<%sb'%length
        ...
        val1.tracking_fmap = s.unpack(str[start:end])
