...
"""
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      buff.write(_struct_I.pack(length))
      pattern = '<%sf'%length
      buff.write(struct.Struct(pattern).pack(*self.sigma_vel))
      _x = self
      buff.write(_get_struct_2f().pack(_x.sigma_width, _x.sigma_height))
      length = len(self.sigma_position)
      buff.write(_struct_I.pack(length))
      pattern = '<%sf'%length
      buff.write(struct.Struct(pattern).pack(*self.sigma_position))
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      self.sigma_vel = s.unpack(str[start:end])
      _x = self
      start = end
      end += 8
      ...
      self.sigma_position = s.unpack(str[start:end])
      return self
